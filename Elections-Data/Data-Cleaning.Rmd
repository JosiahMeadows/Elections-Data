...
pal <- colorBin("Blues", bins = c(1000, 5000, 10000, 100000, 200000, Inf))
  ...
  # Play around with bins to get the shades to be darker. Capture biggest 
  # differences.
 
 
#pal <- colorNumeric("Greens", domain = combined$values)
#domain = counties_merged_sb$totals,   
  
# Trying out more code from the tutorial on website
...
# Read in Florida unemployment data. This dataset is messy.

fl_employment <- read_xlsx("raw_data /LFS_cnty.xlsx", skip = 5) %>% 
  clean_names() %>% 
  drop_na()
